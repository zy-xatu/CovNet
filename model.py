...
class CovarNet(nn.Module):
    def __init__(self):
        ...
        self.cfe = CFE(3, 1)
        self.head = Head()
        self.stem_layer = nn.Conv2d(3, 16, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x1, x2):
        ...
        """
        # x = self.cfe(x1, x2)
        # # for stage in self.stage_layer:
        # #     x = stage(x)
        # #     self.down_sample.append(x)
        # # for _, d2t in enumerate(self.d2t):
        # #     x = d2t(x)
        # #     if _ < len(self.d2t) - 1:
        # #         x = torch.add(x, self.down_sample[len(self.down_sample) - _ - 2])
        # # x = self.head(x1, x)
        x = self.cfe(x1, x2)
        ...
        x = self.d2t4(x_d2t3)
        output = self.head(x1, x).flatten(1)
        return output
...
class CFE(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.conv2 = nn.Conv2d(in_c, out_c, kernel_size=1, stride=1, padding=0)

    def forward(self, x, y):
        x = self.conv1(x)
        y = self.conv2(y)
        mean = self.get_mean(x, y)
        Covar = (x-mean) @ (y-mean).transpose(-1, -2)
        result = torch.cat((x, Covar, y), dim=1)
        return result
...
class C2f(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=1, stride=1, padding=0, deepen_factor = 0.33, deepth=6, e=0.5, add = True):
...
if __name__ == '__main__':
    ...
    image_t0 = torch.randn(64, 3, 256, 256).to(device)
    image_t1 = torch.randn(64, 3, 256, 256).to(device)
