...
class DataProcess(Dataset):
    ...
    # def filelist(self,root_path,file_type=('bmp'or 'img')):
    #     return [os.path.join(root,f) for root,dirs,files in os.walk(root_path) for f in files if f.endswith(file_type)]
    ...
    def __getitem__(self, index):
        ...
        # xml_file = os.path.join(self.ano_path,'/'.join(img_file.split('/')[-2:]).replace('.bmp','.xml'))
        assert os.path.exists(xml_file),"{} 文件没有发现!".format(xml_file)
        # with open("data.txt", mode="a") as f:
        #     f.write(f"img_file:{img_file},\txml_file:{xml_file}\n")
        xy_center = self.get_center(xml_file)
        target = xy_center[0][1] * 256 + xy_center[0][0]
        targets = {}
        targets['label'] = torch.as_tensor(target)
